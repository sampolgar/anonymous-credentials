...
impl<E: Pairing> Signature<E> {
    pub fn sign(
        ...
        let sigma2 = (commitment.cm.add(sk.sk)).mul(u).into_affine();
        Self { sigma1, sigma2 }
    }
    ...
    pub fn verify(
        ...
        let left1 = E::pairing(self.sigma2, pp.g_tilde);
        let right1 = E::pairing(self.sigma1, vk.vk_tilde.add(commitment.cm_tilde));
        assert!(left1 == right1, "Pairing check failed!");

        let left2 = E::pairing(commitment.cm, vk.vk_tilde);
    ...
    pub fn verify_with_pairing_checker(
        ...
        let check1 = PairingCheck::<E>::rand(
            &mr,
            &[
                (&self.sigma2, &pp.g_tilde),
                (
                    &self.sigma1.into_group().neg().into_affine(),
                    &vk_plus_cm_tilde,
                ),
                (
                    &self.sigma1.into_group().neg().into_affine(),
