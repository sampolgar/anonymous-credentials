...
//         // let g1_pairing_points =
//         //     PairingUtils::<Bls12_381>::combine_g1_points(&c_vec, &[a, e, g_inv]);
...
//     fn test_commit_and_prove_knowledge_and_SoK_using_pairingcheck() {
//         // setup keys and signature first
//         let message_count = 6;
//         let mut rng = ark_std::test_rng();
//         let (sk, pk) = keygen::<Bls12_381, _>(&mut rng, &message_count);
...
//         let g2_commitment = <G2Projective as VariableBaseMSM>::msm(&bases, &blindings).unwrap();
//         let gt_commitment: PairingOutput<Bls12_381> =
//             Bls12_381::pairing(sigma_prime_1, g2_commitment);
...
//         // let mut lhs_g1_points = vec![sigma_prime_1];
//         // let mut lhs_g2_points = vec![pk.g2.mul(z_beta).into_affine()];
...
//         // create [(sigmap1, z_beta_affine)]
//         let z_beta_affine = pk.g2.mul(z_beta).into_affine();
//         let mut lhs_pairs: Vec<(&G1Affine, &G2Affine)> = vec![(&sigma_prime_1, &z_beta_affine)];

//         // scale y_g2 by z_alpha
//         // add y, m [(sigmap1, z_beta_affine), (sigmap1, z_alpha1), (sigmap1, z_alpha2)...]
//         let y_g2_scaled: Vec<G2Affine> = z_alpha_i
//             .iter()
//             .zip(pk.y_g2.iter())
//             .map(|(z_i, y_g2)| y_g2.mul(z_i).into_affine())
//             .collect();

//         lhs_pairs.extend(
//             std::iter::repeat(&sigma_prime_1)
//                 .zip(y_g2_scaled.iter())
//                 .take(z_alpha_i.len()),
//         );
...
//         // Create PairingCheck
//         let pairing_check = PairingCheck::<Bls12_381>::rand(&mr, &lhs_pairs, &rhs.0);

//         // Verify the pairing equation
//         assert!(pairing_check.verify(), "SoK verification failed");
//     }
...
//             PairingUtils::<Bls12_381>::multi_miller_loop(prepared_g1, prepared_g2);
