...
// Internal function to convert u32 to little-endian byte array
fn u32_to_le_bytes(i: u32) -> [u8; 4] {
    i.to_le_bytes()
}

// Function that returns an iterator of little-endian byte representations
fn le_bytes_iter(count: u32) -> impl Iterator<Item = [u8; 4]> {
    (0..count).map(u32_to_le_bytes)
}
