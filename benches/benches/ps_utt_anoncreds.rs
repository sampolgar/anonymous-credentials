...
use ps_utt::anon_cred::{AnonCredProtocol, ShowCredential, UserCred as StdUserCred};
use ps_utt::anon_cred_improved::{
    AnonCredProtocolImproved, ShowCredentialImproved, UserCred as ImpUserCred,
};
use ps_utt::signature::{PSUTTSignature, PSUTTSignatureImproved};
use std::time::Duration;

// Standard protocol test setup structure
struct StandardTestSetup<E: Pairing> {
    protocol: AnonCredProtocol<E>,
    user_cred: StdUserCred<E>,
    proof: Vec<u8>,
...
fn setup_standard_protocol(msg_size: usize) -> StandardTestSetup<Bls12_381> {
    ...
    let user_cred = StdUserCred::<Bls12_381>::new_random_messages(&protocol.pp);
...
fn setup_improved_protocol(msg_size: usize) -> ImprovedTestSetup<Bls12_381> {
    ...
    // Generate user credentials
    let user_cred = ImpUserCred::<Bls12_381>::new_random_messages(&protocol.pp);
...
fn benchmark_psutt_std(c: &mut Criterion) {
    ...
    // Configure benchmark parameters (same as BBS+)
    group
        .sample_size(10)
        .measurement_time(Duration::from_secs(10));

    // Message sizes to benchmark (same as BBS+)
...
criterion_group!(
    ...
    targets = benchmark_psutt_std, benchmark_psutt_imp
);
